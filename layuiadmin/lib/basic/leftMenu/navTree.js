layui.define(["element", "jquery"], function(exports) {
	...
	NavTree.prototype.getTreeNodes = function(data, rootValue, props) {
		...
		for(var i = 0; i < data.length; i++) {
			item = data[i] || {};
			var itemId = item[options.idKey] || "",
				itemPid = item[options.pidKey] || "";
	...
	NavTree.prototype.renderNavData = function(navData) {
		...
		for(var i in data) {
			if(data.hasOwnProperty(i)) {
				item = data[i] || {};
				var itemId = item[options.idKey],
					itemCode = item[options.codeKey],
					itemTitle = item[options.titleKey],
					itemHref = item[options.hrefKey],
					itemIcon = item[options.iconKey],
...
//				sideHtml.push('" href="');
	...
	NavTree.prototype.navLink = function(navParm, isRedirect) {
		...
		if(!navParm && config.currentNavKey) {
			...
			if(storage) {
				var currentNav = JSON.parse(storage) || {
					code: "",
					href: ""
				};
		...
		$.each(ancestors, function(index, elem) {
			var _self = $(this),
				code = _self.children("a").attr("data-code");
		...
		if(isRedirect) {
			var href = navObj.attr("href") || config.home,
