...
//        children: "children",
  ...
  $.fn.zTree = {
    ...
    init: function (obj, zSetting, zNodes,handerSetting) {
      var setting = tools.clone(_setting);
//    setting.
			var handerSettings = handerSetting;
//			options.setting.data.key.
//			setting.data.key.
			for(keys in handerSettings){
				setting.data.key[keys] = handerSettings[keys]
			}
...
//      var sda  = settings;
//		settings.data.key.isParent = options.setting.data.key.isParent||settings.data.key.isParent;
//		settings.data.key.name = options.setting.data.key.name||settings.data.key.name;
//		settings.data.key.children = options.setting.data.key.children||settings.data.key.children;
		var handerSetting =  {}
		try{
			handerSetting = options.setting.data.key
		}catch(x){
			
		}
//		var handerSetting = options.setting.data.key || {}
//		var w_setting = tools.clone(_setting);
    var a = {
      init: function(){
//      $.ajax({
//        url: data,
...
//        success: function (d) {
            ...
            $.fn.zTree.init($('#' + TREE_SELECT_BODY_ID), a.setting(), d,handerSetting);
            if (search) {
              a.searchParam();
            }
...
//      });
      ...
      onClick: function(event, treeId, treeNode){
      	var names = handerSetting.name||'name'
      	var ids = handerSetting.id||'id'
        var name = treeNode[names],
            id = treeNode[ids],
