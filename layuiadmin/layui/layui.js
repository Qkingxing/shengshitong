...
! function(e) {
	...
	var t = document,
		...
		u = {
			...
			//自扩展
			echarts:'extend/echarts/echarts', //echarts 核心包
		    echartsTheme:'extend/echarts/echartsTheme', //echarts 主题
		      
		    soulTable:'extend/soulTable/soulTable',//soulTable扩展
		    ...
		    //下拉列表
		    selectM:'extend/select/selectM',
		    selectN:'extend/select/selectN',
		    //多选下拉
		    xmSelect:'extend/xm-select/xm-select',
		    
		    //图片
		    
		    move:'extend/imgUpload/move',
		    cropper:'extend/imgUpload/cropper',
		    croppers:'extend/imgUpload/croppers',
		    publishImg:'extend/imgUpload/publishImg',
		    //treeselect
		    treeSelect:'modules/treeSelect',
		    //tabletree
		    tableEdit:'modules/tableEdit',
...
//		var v = (u[d] ? f + "lay/" : /^\{\/\}/.test(p.modules[d]) ? "" : n.base || "") + (p.modules[d] || d) + ".js";
		var v ,wstr = "extend";
//		if(u[d] ? f + "lay/" : /^\{\/\}/.test(p.modules[d])){
//			v =  (p.modules[d] || d) + ".js";
//		}else if(u[d] ? f + "extend/" : /^\{\/\}/.test(p.modules[d])){
//			v =  (p.modules[d] || d) + ".js";
//		}else{
//			v =  (n.base || "") + (p.modules[d] || d) + ".js";
//		}
		if(u[d]){
			if((p.modules[d]).indexOf('extend')>-1){
				v = f + "../lib/"
			}else{
				v = f + "lay/"
			}
